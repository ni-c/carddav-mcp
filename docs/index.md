...
<svg viewBox="0 0 720 268" role="img" aria-labelledby="arch-title arch-desc">
  ...
  <!-- Two lines rather than one: "Radicale, Baikal, Nextcloud, …" is about
       168px at 11.5px and the box is 150 wide, so on one line it spilled past
       both edges of its own box. -->
  <rect class="node" x="550" y="70" width="150" height="80" rx="10" />
  <text class="label-title" x="625" y="100" text-anchor="middle">CardDAV</text>
  <text class="label-muted" x="625" y="120" text-anchor="middle">Radicale, Baikal,</text>
  <text class="label-muted" x="625" y="136" text-anchor="middle">Nextcloud, …</text>
  ...
  <!-- Stacked, not joined with separators. The gap between the two boxes is
       100px wide and the one-line version was ~150px, so it was drawn straight
       across the right edge of carddav-mcp and into the CardDAV box, on top of
       the text already there. Each verb on its own line fits the gap with room
       to spare and reads as a list rather than as a sentence. -->
  <text class="label-muted" x="496" y="128" text-anchor="middle">PROPFIND</text>
  <text class="label-muted" x="496" y="144" text-anchor="middle">REPORT</text>
  <text class="label-muted" x="496" y="160" text-anchor="middle">PUT</text>
